...
This page walks through each type of token includes examples for each option.
...
Spacing tokens scale with the screen width, becoming slightly more spread out
as the screen widens. The variables for spacing are prefixed with `--space-`
and are implemented as T-shirt sizes between `3xs` and `3xl`, plus 2 additional
sizes: `none` and `body`. The `none` size is used to represent no spacing, while
`body` is used for the padding around the edges of the app.
